...
def plot(target, forecast, prediction_length, prediction_intervals=(50.0, 90.0), color='g', fname=None):
    """
        Plotting function for time series predictions.

        Parameters:
        - target: Actual time series data.
        - forecast: Forecasted time series data.
        - prediction_length: Length of the predicted future.
        - prediction_intervals: Tuple of prediction intervals (default: (50.0, 90.0)).
        - color: Color of the plot (default: 'g').
        - fname: File name for saving the plot (default: None).
    """
    label_prefix = ""
    rows = 4
    cols = 4
...
if config.train:
    train_output = estimator.train_model(dataset_train, num_workers=4)  # Set num_workers > 0 to enable multiprocessing
...
output_directory = os.path.dirname(config.path)
os.makedirs(output_directory, exist_ok=True)

# Save the model
if config.save and config.train:
    save_path = config.path[:-4] + str(metrics['CRPS-Sum:']) + config.path[-4:]

    print(f"Saving model to: {save_path}")

    torch.save(train_output.trained_net.state_dict(), save_path)
